...
| M8 — Snacktum Snacktorum rebrand | rebrand "Top Dog" → the hot-dog CULT app; app shell + nav, auth cluster (sign-in / reset / ritual sign-up), profile redesign, "Anoint" re-theme, error/404, cult theme | scoped — design phase (build blocked) | **User-pivot milestone, SKIN NOT SKELETON** (user-facing copy/markup/CSS/new pages only — NO code-identifier or infra rename; "Top Dog" → "The Anointed Wiener" is **copy only**; no migration / no new dependency / no new decision row expected). Scoped in PR #90; the **auth cluster + app shell designs delivered + committed in PR #91** (`design/pages/` Log In / Reset Password / Snacktum Onboarding ritual / App Chrome shell; `design/avatars/` 5 SVG sigils; `design/assets/` brand marks + favicons; `design/page-design-prompts.md` 11 prompts for the still-unbuilt in-app pages). **No build started — every task `blocked`, gated on the user's explicit "go".** RESOLVED (2026-06-18): the ritual sign-up rite (Summoned → Inscribe Thy Name → Choose Thy Sigil → Renounce the Patty → Received, **absorbing** onboarding), the 5-sigil avatar (stored as a sigil id in the existing `avatar_path` — **no migration / no storage upload**; user-uploaded avatars deferred), the dark-temple theme (`#17120e` + Mustard Gold `#E0A82E`), self-hosted Cinzel + Cormorant Garamond woff2 (no dependency), and a 6-digit-OTP reset flow (not a magic link; local emails → Mailpit). STILL OPEN: OQ-2 (the four Anoint sub-decisions) + OQ-5 (six TBD page names) + generating the remaining in-app page designs. Design-ready (awaiting the build go-ahead): TASK-080 shell, TASK-082 sign-in, TASK-083 reset, TASK-084 ritual, TASK-087 theme. See [[tasks/milestone-08-snacktum-snacktorum-rebrand]]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
