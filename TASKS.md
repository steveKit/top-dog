...
> **Task set (re-scoped): 12 complete + 5 pending.** Complete: TASK-087 (theme) + TASK-080
...
> dog-detail rebuild, decision-#27 preserved). Pending:
> per-page rebuilds — **TASK-097** Epistles+Whispers, **TASK-098** Summon, **TASK-099**
...
> landing). 11/16 complete** (theme + shell + the auth cluster: sign-in / forgot /
> reset — auth is functional end-to-end — + the onboarding rite + the foundational slug
> refactor + The Procession + The Shrine + Anoint re-theme + Your Litter + The Relic). TASK-090 moved the in-app route prefix `/app` →
...
> kept byte-identical; no migration / dep / decision row. The remaining work is the per-page
> leaf rebuilds from the delivered mockups (**TASK-097 Epistles+Whispers next, then Summon,
> Tribunal, Catechism, Lost Pilgrim**).
