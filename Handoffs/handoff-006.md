...
Resumed from [[Handoffs/handoff-005]] (Milestone **M2 — Voting & Top Dog Engine**
...
- `TASKS.md` — **migrated.** Now the index/dashboard for the per-milestone layout
  ...
  this session.)_
- `tasks/*` — **new (6 milestone files + `discovered.md` + `deferred.md`).** The
  exploded per-milestone queue.
- `TASKS-ARCHIVE.md` — **new.** Grandfathered M0 + M1 completed-task archive.
...
- `Handoffs/handoff-005.md` — **new** (merged this session).
...
([[tasks/discovered]], DW-001..008) is unchanged save for dispositions already
...
1. **TASK-023 — Top Dog badge UI** [`P2`, `S`] — the **LAST** task in M2. Renders
   ...
   Lives in [[tasks/milestone-02-voting-top-dog-engine]]. **Completing it CLOSES
...
2. **After M2 closes: M3 — Reactions & Per-Dog Stats** (TASK-030/031), or per user
   selection. See [[tasks/milestone-03-reactions-per-dog-stats]].
...
- [[TASKS]] — **now the index/dashboard**; note the new layout
  ([[TASKS-ARCHIVE]] for completed M0/M1, [[tasks/discovered]], [[tasks/deferred]]).
- [[tasks/milestone-02-voting-top-dog-engine]] — the active milestone; **TASK-023
