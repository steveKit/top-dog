...
last closed milestone (see [[workflow/handoffs/handoff-009]]); M4 has not started. Three
things happened:
...
action. The reusable prevention (now in [[PROJECT]], [[workflow/memory/MEMORY]], and the
...
- `workflow/memory/MEMORY.md` — MODIFIED (this handoff): new **Deploy / Ops Patterns**
...
- `CLAUDE.md` — MODIFIED (this handoff): Project Map latest-handoff pointer →
  `[[workflow/handoffs/handoff-010]]`; keep-alive gotcha refined to record the two distinct
  red-workflow failure modes (ping-fail = secrets vs RPC-404 = hosted schema drift).
- `workflow/handoffs/handoff-010.md` — NEW (this file).
...
1. **[P1] Activate M4 — Mustard Mechanic** ([[workflow/tasks/milestone-04-mustard-mechanic]]).
...
- [[workflow/tasks/milestone-04-mustard-mechanic]] — the next milestone (TASK-040 → 042).
...
- [[workflow/memory/MEMORY]] — the new Deploy / Ops pattern for diagnosing a red keep-alive.

## Library Candidates

_None — this session touched only `.mise.toml` and `PROJECT.md` (plus bookkeeping)._

See [[workflow/handoffs/handoff-009]] for prior session context.
