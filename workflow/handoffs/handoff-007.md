...
Resumed from [[workflow/handoffs/handoff-006]] (Milestone **M2 — Voting & Top Dog Engine**
...
2. **Badge reuses the pure `selectTopDog` comparator (already recorded).** The
   ...
   the TASK-023 Notes block in [[workflow/tasks/milestone-02-voting-top-dog-engine]] and the
...
- `PROJECT.md`, `workflow/tasks/discovered.md`,
  `workflow/tasks/milestone-02-voting-top-dog-engine.md` — **modified** (TASK-023
...
- `CLAUDE.md` — **modified** (this handoff: Project Map latest-handoff pointer →
  handoff-007).
- `workflow/memory/MEMORY.md` — **modified** (this handoff: orphan-by-design wiring-audit
  pattern).
- `workflow/handoffs/handoff-007.md` — **new** (this file).
...
- **M2 close is gated on a user decision** about the vote-casting UI. Three paths:
  ...
  selected until the user decides.** See DW-009 in [[workflow/tasks/discovered]] and the
  held-open note in [[workflow/tasks/milestone-02-voting-top-dog-engine]].
...
- **Cross-project (user-owned, NOT this repo):** the core-architect should add the
  ...
  from [[workflow/handoffs/handoff-006]], still pending.

## Discovered Work

One **new** item was logged this session (full text in [[workflow/tasks/discovered]]):
...
1. **Decide the vote-casting UI path to close M2** [`P1`] — draft TASK-024
   ...
   is the gate on everything else in M2. See DW-009 in [[workflow/tasks/discovered]] and the
   held-open note in [[workflow/tasks/milestone-02-voting-top-dog-engine]].
2. **After M2 closes: M3 — Reactions & Per-Dog Stats** [`P2`] — cosmetic emoji
   reactions + peak votes. See [[workflow/tasks/milestone-03-reactions-per-dog-stats]].
...
- [[workflow/tasks/milestone-02-voting-top-dog-engine]] — Active Tasks held-open note +
  completed TASK-023.
