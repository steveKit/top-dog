...
// redirected to /app/feed and never sees the flagged-dog queue) and its `rule` action
...
//   - load: unauth -> redirect /sign-in; a NON-Top-Dog -> redirect /app/feed (never
...
describe('court load (crown gate)', () => {
	...
	it('redirects a NON-Top-Dog to /app/feed; never lists flagged dogs (UI gate half)', async () => {
		...
		expect(isRedirect(thrown)).toBe(true);
		expect((thrown as { location: string }).location).toBe('/app/feed');
		// The flagged queue is the Top Dog's alone: a non-Top-Dog never reaches it.
		expect(listFlaggedDogs).not.toHaveBeenCalled();
		expect(getSignedUrl).not.toHaveBeenCalled();
	});

	it('redirects a profile-less (null) viewer to /app/feed (treated as not Top Dog)', async () => {
