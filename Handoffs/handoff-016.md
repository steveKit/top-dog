...
- **PR #90 `docs`** — **Milestone M8 scoped** (design-blocked). The full rebrand/
  redesign milestone exploded into [[tasks/milestone-08-snacktum-snacktorum-rebrand]]
...
[[tasks/milestone-08-snacktum-snacktorum-rebrand]] § Open Questions, marked RESOLVED).
...
- `tasks/milestone-08-snacktum-snacktorum-rebrand.md` — **NEW** (PR #90; updated
...
- `CLAUDE.md` — MODIFIED (**this handoff**): Project Map latest-handoff pointer →
  `[[Handoffs/handoff-016]]`; a durable **Local dev (WSL)** note (`pnpm dev --host`
...
- `Handoffs/handoff-016.md` — **NEW** (this file).
...
- **Main-commit / push hook boundary** — the pre-tool-safety hook blocks the director
  ...
  **user's hand** (the known `chore/*`-branch constraint in [[memory/MEMORY]]).

## Discovered Work

**No new discovered-work items this session** — it was a pivot/scoping/design session,
not an implementation one. The standing open backlog (per [[tasks/discovered]]) is
...
- [[tasks/milestone-08-snacktum-snacktorum-rebrand]] — the M8 spec: the HARD SCOPE
...
See [[Handoffs/handoff-015]] for prior session context (the M7 close).
