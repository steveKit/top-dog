...
| M3 — Reactions & per-dog stats | cosmetic reactions, peak votes                                                      | in progress | TASK-030 (cosmetic reactions, PR #43 `b27dc63`) done — `hotdog_reactions` table + owner-scoped RLS insert/delete (deliberately not an RPC; no denormalized counter, render-time counts, so it structurally cannot affect ranking), `ReactionBar` wired into `/app/feed`. TASK-031 (per-dog stats, PR #45 `e1ffa0e`) done — display/wiring only (no schema change): new `src/lib/features/hotdogs/detail.ts` query + `/app/dogs/[id]` detail route (404/500 mapping, signed-URL graceful degradation, read-only reaction summary, Stats block, `<TopDogBadge>`), `peak_votes` surfaced on the feed/tiles. **M3 stays open:** TASK-032 (E2E hardening — `/app/feed` + `/app/dogs/[id]` flows, promoted from DW-011) remains. See M3 progress notes above                                                                                                                                                                                                                                   |
