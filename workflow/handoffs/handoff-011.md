...
* `PROJECT.md`, `TASKS.md`, `workflow/tasks/milestone-04-mustard-mechanic.md`,
  `workflow/tasks/discovered.md` (DW-017), `README.md`, `CLAUDE.md` — bookkeeping (largely
  landed in #54/#56/#58 this session).
* `CLAUDE.md` — MODIFIED (this handoff): Project Map latest-handoff pointer →
  `[[workflow/handoffs/handoff-011]]`.
* `workflow/handoffs/handoff-011.md` — NEW (this file).
...
- ⚠️ **HEADLINE — the hosted-push gate is PENDING.** The two new M4 migrations
  ...
  **red** — the exact hosted-schema-drift class from handoff-010 (which took the
  ...
  - Diagnostic reminder ([[workflow/memory/MEMORY]] Deploy/Ops): if the workflow goes red,
...
- [ ] **DW-017** — the `spray` form action coerces a missing/empty `x`/`y` to `0`
      ...
      TASK-041 review. Disposition `open` in [[workflow/tasks/discovered]] (minor, deferred).
...
2. **[P1] Activate M5 — Walls & DMs** ([[workflow/tasks/milestone-05-walls-dms]]). M5 is
   ...
   cross-member E2E lesson from M3 ([[workflow/memory/MEMORY]]) applies directly to walls
...
- [[workflow/tasks/milestone-05-walls-dms]] — the next milestone (pre-exploded).
...
- [[workflow/memory/MEMORY]] — the cross-member-E2E and Deploy/Ops (keep-alive drift)
...
See [[workflow/handoffs/handoff-010]] for prior session context.
