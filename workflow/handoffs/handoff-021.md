...
- **PR #143 — workflow v5 config-sync + design relocation.** Ran `/migrate-workflow` in
  ...
  `testing-paradigm: adaptive` key to the Testing Strategy section, and created [[workflow/handoffs/INDEX]]
...
- **PR #144 — living-doc design paths + a pre-existing prettier fix.** Retargeted the design-path
  ...
  fixed a **pre-existing** prettier drift in [[workflow/tasks/discovered]] (already red on `main`, unrelated
...
1. **Migrated to workflow v5.** `workflow-version: 5` is now stamped in [[CLAUDE]], the
   ...
   handoff set has an append-only [[workflow/handoffs/INDEX]].
...
- `CLAUDE.md` — **MODIFIED** (PR #143 + #144, and this handoff): `workflow-version: 5` stamped;
  ...
  handoff" pointer → `[[workflow/handoffs/handoff-021]]` (this handoff).
...
- `.prettierignore`, `eslint.config.js` — **MODIFIED** (PR #143): design-source ignore `design/` →
  `docs/design/`.
- `workflow/tasks/discovered.md` — **MODIFIED** (PR #144): pre-existing prettier drift fixed (formatting
  only, no content change).
- `workflow/handoffs/INDEX.md` — **NEW** (PR #143), + **MODIFIED** (this handoff): created as the
...
- `design/**` → `docs/design/snacktum-snacktorum/**` — **MOVED** (PR #143, 31 files, R100/R099
  ...
  `status:` header.
- `workflow/handoffs/handoff-021.md` — **NEW** (this file).
...
Two process notes (recurring, both in [[workflow/memory/MEMORY]]): this repo's agents can't run the live
...
DW-031 / DW-033) and the older standing backlog all carry open. See [[workflow/tasks/discovered]].
...
- [[workflow/tasks/discovered]] — DW-041 and the carried M8 tweak-session backlog.
...
See [[workflow/handoffs/handoff-020]] for prior session context (M8 close — Summon / Tribunal / Catechism /
