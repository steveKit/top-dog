...
DW-011 (no E2E for the `/feed` route) — see [[workflow/tasks/discovered]].
...
[[workflow/tasks/discovered]]). A vote-casting UI task must land and re-pass the wiring
audit before M2 can close.

**Discovered during this task:** DW-009 (no vote-casting UI consumer for
`castVote` / `removeVote`) — see [[workflow/tasks/discovered]].
