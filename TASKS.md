...
> **Task set (re-scoped): 16 complete + 1 pending.** Complete: TASK-087 (theme) + TASK-080
...
> byte-identical, the double gate + anonymity preserved) + TASK-100 (The Catechism — PR #138;
> static how-it-works rebuild + leaf `help`→`catechism`, accuracy-checked — corrected the
> mockup's stale Anoint-decay copy to ~6h, closed DW-038). Pending:
> the one new page — **TASK-101** The Lost Pilgrim (error/404).
> (The above 16 includes TASK-094-R, a derived sub-module not counted in the milestone `/16`,
> so the headline rollup is **15/16**.) The old
...
> landing). 15/16 complete** (theme + shell + the auth cluster: sign-in / forgot /
> reset — auth is functional end-to-end — + the onboarding rite + the foundational slug
> refactor + The Procession + The Shrine + Anoint re-theme + Your Litter + The Relic + Epistles/Whispers + Summon a Frank + The Tribunal + The Catechism). TASK-090 moved the in-app route prefix `/app` →
...
> / dep / decision row. **TASK-094 (Anoint) then LANDED 2026-06-22** (PR #124, `645373a`):
> the Shrine mustard surface re-themed to a splat, overlay decay 24h→6h, a persisting
> render-derived anoint→wall-notice, and the **M8 migration** retiring `prune_mustard_sprays`
> (+ keep-alive prune step removed in lockstep) — **architecture decision #29 (mustard_sprays
...
> decision row. **TASK-096 (The Relic) then LANDED 2026-06-22** (PR #130, `d07315f`): the
> dog-detail `+page.svelte` rebuilt in place at `litter/[id]`, the decision-#27 signed-URL load
> kept byte-identical; no migration / dep / decision row. **TASK-097 (Epistles + Whispers) then
> LANDED 2026-06-22** (PR #132, `8764287`): both DM pages rebuilt + the leaf `messages` →
> `epistles` (+ `epistles/[handle]`), both servers preserved; no migration / dep / decision row.
> **TASK-098 (Summon a Frank) then LANDED 2026-06-23** (PR #134, `dc7a229`): the invite
...
> **TASK-100 (The Catechism) then LANDED 2026-06-23** (PR #138, `e42087a`): the static
> how-it-works `+page.svelte` rebuilt from `design/pages/The Catechism.dc.html` as The
> Catechism (six illuminated Article sections + a doxology) + the final leaf `help` →
> `catechism` (the whole folder `git mv`'d; a static page, so there is no `+page.server.ts`).
> **Accuracy was the point:** the mockup's stale "fades over about a day" Anoint copy was
> corrected to the real **~6h** (`MUSTARD_LIFESPAN_MS`, decision #29), and FALSE WITNESS (7d)
> / HERETIC (permanent) / vote+crown / reactions were all cross-checked against
> `voting/ranking.ts` + `reports/verdict.ts`; the dangling kennel link was dropped (**DW-038
> closed**). With it, **all in-app leaves are now renamed** — no pre-rename leaves remain. No
> migration / dep / decision row / code-identifier rename.
> The remaining work is the one new page from the delivered mockups (**TASK-101 Lost Pilgrim**,
> the root `+error.svelte`) — completing it closes the milestone.
