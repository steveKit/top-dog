...
| M4 — Mustard mechanic          | spray + render-time decay + >24h prune                                              | active   | In progress — 1 of 3 tasks done. TASK-040 (mustard decay math, PR #53 `5afd0da`) — pure render-time `src/lib/features/mustard/decay.ts` (`MUSTARD_LIFESPAN_MS` + `mustardOpacity`, full→0 over 24h, clock-skew clamp), TDD-first (19 unit cases), realizing decision #15; orphan-by-design with TASK-041 as the named consumer, no schema/RLS/RPC change. TASK-041 (spray + render) and TASK-042 (>24h prune job, wired into keep-alive — must `supabase db push` its migration to hosted) remain. See M4 progress notes above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
