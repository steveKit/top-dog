...
// funnels the profile-less user to /app/onboarding — proving both that the
// credentials authenticated AND that the auth cascade routed onward (we do NOT
// bypass the guard). The credentials come from the shared bootstrap-fixtures
...
test('@smoke a seeded user signs in through the real form and reaches the app', async ({
	...
	// (3) The action redirects to /app; the guard funnels this profile-less user
	// to onboarding. Reaching either confirms the session was established and the
	// guard cascade ran (we never bypass it).
