...
**Phase:** M0–M8 complete · **M8 (Snacktum Snacktorum rebrand) — COMPLETE (16/16), closed 2026-06-23, tag `milestone-08-snacktum-snacktorum-rebrand`** (activated 2026-06-19; **re-scoped 2026-06-19**; closed 2026-06-23 with TASK-101 The Lost Pilgrim). The milestone delivered: the **auth cluster** (TASK-082 sign-in + TASK-083 password recovery + TASK-092 the Snacktum Onboarding rite at `/sign-up`) — functional end-to-end; the base cult **theme** (TASK-087) + persistent **app shell** (TASK-080, plus the ad-hoc App Chrome rebuild PR #119); the foundational **slug refactor** (TASK-090) moving the in-app route prefix `/app` → `/snacktum-snacktorum`; and **eight rebuild-from-design in-app pages** — The Procession (TASK-091, feed → `procession`), The Shrine (TASK-093, profile → `shrine/[handle]`, + a derived stat ledger), the "Anoint" mustard re-theme (TASK-094 — the one M8 migration retiring the prune → `mustard_sprays` is append-only, **architecture decision #29**), Your Litter (TASK-095, dogs → `litter`), The Relic (TASK-096, dog-detail at `litter/[id]`, byte-identical `+page.server.ts` preserving the decision-#27 signed-URL load), Epistles + Whispers (TASK-097, messages → `epistles`), Summon a Frank (TASK-098, invite → `summon`, + a copy-to-clipboard affordance), The Tribunal of the Holy Tube (TASK-099, court → `tribunal`, byte-identical server preserving the double gate + anonymous aggregate), and The Catechism (TASK-100, help → `catechism`, accuracy-checked) — **every in-app leaf is now renamed** (procession / shrine / litter / epistles / summon / tribunal / catechism). The Shrine cluster also closed with **TASK-094-R The Reliquary** (a purely DERIVED badge/honors module + shelf, not counted in the `/16`). The final task, **TASK-101 The Lost Pilgrim** (PR #140 `0d6a63d`), added the brand-new root `+error.svelte` boundary — driven by `page.status` + `page.error`, it renders only friendly copy + the numeric status, never `page.error.message` (L2 no-leak, structural). **No new dependency, no new migration beyond TASK-094's, and no new architecture-decision row after #29** — the whole rebrand stayed **skin not skeleton** (copy / markup / route slugs only; no table/RPC/TS-symbol or infra rename; "Top Dog" → "The Anointed Wiener" is copy only; decisions #1–#29 + L2 preserved). **Next milestone: M9 — Operator / Admin Dashboard (stub — activate via the planner).**
**Maintenance (2026-07-08, no product tasks):** migrated to **workflow v5** (`workflow-version: 5`
plus a machine-readable `testing-paradigm: adaptive` key in [[CLAUDE]], and a new append-only
[[workflow/handoffs/INDEX]]) and relocated the design artifacts from repo-root `design/` →
`docs/design/snacktum-snacktorum/` per [[design-intake]] (`git mv`, history preserved). The living
docs ([[CLAUDE]], [[PROJECT]]) were swept to the new path; frozen handoff / milestone archives and
the `src/**` code comments deliberately keep `design/` as historical provenance. No milestone or
task status changed, **no new decision row — the table stays at #29**. See [[workflow/handoffs/handoff-021]].
**Last Updated:** 2026-07-08

> **‼️ M8 RE-SCOPE (2026-06-19, user-directed) — rebuild-from-design + re-slug.** The
> remaining M8 pages are now **REBUILT FROM their design mockups** (`docs/design/snacktum-snacktorum/pages/*.dc.html`
...
> onboarding rite at `/sign-up`). See [[workflow/tasks/milestone-08-snacktum-snacktorum-rebrand]] § Slug Map.
...
the app shell are fully designed and committed** (`docs/design/snacktum-snacktorum/pages/` mockups: Log In, Reset
Password, the Snacktum Onboarding ritual, the App Chrome shell; `docs/design/snacktum-snacktorum/avatars/` 5 SVG
sigils; `docs/design/snacktum-snacktorum/assets/` brand marks + favicons; `docs/design/snacktum-snacktorum/page-design-prompts.md` 11
...
**rebuilt from `docs/design/snacktum-snacktorum/pages/The Procession.dc.html`** as The Procession — a
...
**rebuilt from `docs/design/snacktum-snacktorum/pages/The Shrine.dc.html`** as The Shrine — a skin-not-skeleton pass
...
`+page.svelte` was rebuilt from `docs/design/snacktum-snacktorum/pages/Your Litter.dc.html` as Your Litter — a
...
markup-only rebuild.** The dog-detail `+page.svelte` was rebuilt from `docs/design/snacktum-snacktorum/pages/The Relic.dc.html`
...
**The twelfth M8 task has now landed: TASK-097 — Epistles + Whispers (12/16; PR #132
`8764287`, merged 2026-06-22), the fifth rebuild-from-design in-app page and the DM
cluster's single task — both DM pages rebuilt at once.** The inbox `+page.svelte` was rebuilt
from `docs/design/snacktum-snacktorum/pages/Epistles.dc.html` as Epistles and the thread `+page.svelte` from
`docs/design/snacktum-snacktorum/pages/Whispers.dc.html` as Whispers — a skin-not-skeleton pass that **preserved both
`+page.server.ts` files** and re-wired their plumbing into the new markup. The whole
`messages` folder was `git mv`'d → `epistles` (carrying the `[handle]` thread subfolder), so
the inbox is now at `/snacktum-snacktorum/epistles` and the thread at
`/snacktum-snacktorum/epistles/[handle]` (param preserved); **only the `messages` leaf moved**
(`invite` / `court` / `help` remain pre-rename, riding their own rebuilds). **Both servers are
preserved with the lightest possible touch:** the inbox server is a **pure rename** (load
untouched), and the thread server's ONLY change is the self-thread redirect literal
`/messages` → `/epistles` (the path moved, so its own redirect target had to move with it) —
the load-bearing wiring stays verbatim: the **conversation-scoped privacy SELECT**, the
**`read_at`-only mark-read write boundary** (decision #24's column-grant lockdown — mark-read
may touch only `read_at`, never forge a message), the **bounded reads** (DW-018/DW-025's
`.limit(50)`), and the **sender-pinned send** (`sender_id = auth.uid()`, un-forgeable). The
compose box adopts the **themed-validation CANON** (the empty-able message field): the textarea
is **nested inside its `<label>`** (so `fieldLabel()`'s `closest('label')` resolves the visible
label rather than falling back to the field `name`), and `validationMessage.ts` gained a new
**prefix-matched "Whisper unto …" themed message** (with over-match guards) rather than a
hand-written string at the call site. Every `messages` → `epistles` reference across the rebuilt
pages, their server wiring, and the live docs was updated (a grep for the `messages` route
segment now returns zero). **No migration, no new dependency, no new decision row, no
`{@html}`** (decisions stay #1–#29, L2 preserved). Reviewer APPROVE, **0 fix cycles**; `pnpm
...
`/snacktum-snacktorum` hub (which 307-redirects to `/procession`) — logged DW-038, to be fixed
naturally when TASK-100 rebuilds The Catechism.

**The thirteenth M8 task has now landed: TASK-098 — Summon a Frank (13/16; PR #134
`dc7a229`, merged 2026-06-23), the sixth rebuild-from-design in-app page.** The invite
`+page.svelte` was rebuilt from `docs/design/snacktum-snacktorum/pages/Summon a Frank.dc.html` as Summon a Frank — a
skin-not-skeleton pass that rebuilt only the page's **main content** (the header/nav chrome
stays owned by the app shell). The mockup's DSL `<sc-if>` states were ported to **Svelte 5
runes** bound to the page's REAL form contract — the four states **idle → minting → minted →
error** driven from the actual `?/create` action result and the `use:enhance` lifecycle, not
a static mock — and a **copy-to-clipboard affordance** (net-new over the original page) was
added for the minted invite link (`navigator.clipboard.writeText` + a legacy `execCommand`
fallback + a transient "Copied ✓"). The whole `invite` folder was `git mv`'d → `summon` as an
**R100 rename**, so the **`+page.server.ts` is byte-identical** — the load-bearing `?/create`
invite-mint action (decisions #17/#22/#23 — single-use invites, the consumed-once guard keyed
on `consumed_at`, owner-scoped issuance) is wholly untouched; the invite is now at
`/snacktum-snacktorum/summon` (only the `invite` leaf moved — `court`, `help` remain
pre-rename). The co-located test was renamed `invite-action.test.ts` → `summon-action.test.ts`
(only its `describe` label + header comment rescoped). **No migration, no new dependency, no
new decision row, and no code-identifier rename** (`$lib/features/invites` and every DB/RPC
identifier stay as-is; decisions stay #1–#29, L2 preserved). Reviewer APPROVE, **0 fix
cycles** — the two findings were minor, optional, and additive over the original page (the
"Copied ✓" feedback lacks an `aria-live`/`role="status"` region; the `execCommand` fallback
doesn't restore focus to the copy button), deferred to the visual-tweak session, not fixed.
`pnpm check` 0/0, `pnpm lint` clean, `pnpm test` 946/946, `@smoke` 5/5, `@security` 93/93.
Discovered: the Summon page has **no inbound nav link**, and the app shell's "＋ Summon a
Frank" header button actually targets **upload** (`/snacktum-snacktorum/litter`), not this
invite page — a label/reachability clash logged DW-039, to be resolved in the visual-tweak
session. **Next: the three remaining leaf-page rebuilds** (TASK-099 Tribunal → TASK-100
Catechism → TASK-101 the Lost Pilgrim error page).
...
[[workflow/tasks/milestone-08-snacktum-snacktorum-rebrand]].
...
(`(protected)/snacktum-snacktorum/+layout.svelte`) to match `docs/design/snacktum-snacktorum/pages/App Chrome.dc.html`
...
**The fourteenth M8 task landed: TASK-099 — The Tribunal (14/16; PR #136 `94de595`, merged
2026-06-23), the seventh rebuild-from-design in-app page.** The court `+page.svelte` was rebuilt
from `docs/design/snacktum-snacktorum/pages/The Tribunal.dc.html` as The Tribunal of the Holy Tube — a skin-not-skeleton
pass that rebuilt only the page's main content (the header/nav chrome + champion sub-bar stay
owned by the app shell) and **preserved the `+page.server.ts` byte-identical** (the whole `court`
folder was `git mv`'d → `tribunal` as an R100 rename, so the adjudication server moved without a
diff). The mock's per-case visual states were mapped onto the **REAL `dog.verdict`** (not a static
mock): `null` → the ⚠ ACCUSED police-tape + both ruling buttons; `confirmed_hamburger` → the reused
`ConfirmedHamburgerStamp` + a Hamburger Heretic treatment; `not_a_hamburger` → a False Witnesses
treatment. Every load-bearing security invariant stays verbatim — the **double gate** (the UI crown
redirect AND the `render_burger_verdict` SECURITY DEFINER RPC's authoritative re-check on the
non-client-writable `is_current_top_dog`, decision #25), the **anonymous flagged-dog aggregate**
(service-client-after-gate with the decision #27/#33 signed-URL handling), and the `rule` verdict
action — and reporter anonymity is intact by construction. The court is now at
`/snacktum-snacktorum/tribunal` (leaf `court` → `tribunal`). No migration / dep / decision row /
code-identifier rename. Reviewer APPROVE, **0 fix cycles**. `pnpm check` 0/0, `pnpm lint` clean,
`pnpm test` 946/946, `@smoke` 5/5, `@security` 93/93. Discovered DW-040 (the presentational CSS
class `shell-nav-court` left as-is — a cosmetic naming nit for the tweak session).

**The fifteenth M8 task landed: TASK-100 — The Catechism (15/16; PR #138 `e42087a`, merged
2026-06-23), the eighth and final rebuild-from-design in-app page.** The static how-it-works
`+page.svelte` was rebuilt from `docs/design/snacktum-snacktorum/pages/The Catechism.dc.html` as The Catechism — six
illuminated "Article" sections in the cult voice (Of the Vote and the Crown / Powers of The
Anointed Wiener / Of Reactions / Of the Anointing / Of Walls and Epistles / The Tribunal of the
Holy Tube) closing on a doxology. Only the page's main content was rebuilt (the header stays in
the app shell). The whole `help` folder was `git mv`'d → `catechism`; the page is **static** (no
`load`, no `actions`), so there is **no `+page.server.ts`** to preserve — the move is the markup
rebuild plus the slug change. **‼️ The headline is accuracy, not skin:** the mockup itself had
drifted, describing the Anoint mustard as fading "over about a day", and the implementer
**corrected this to ~six hours** to match `MUSTARD_LIFESPAN_MS = 6h` (the single source of truth,
shortened from 24h by TASK-094 / decision #29). Every other mechanic-bearing line was cross-checked
against source rather than the mock — **FALSE WITNESS** fades over **7 days**
(`LIAR_BRAND_WINDOW_MS`) and **HERETIC is permanent** (both vs `reports/verdict.ts`); the
vote/crown rules (single movable vote, no self-vote, most votes → the crown, strict sticky
tie-break, days-as-champion tally) and the report → ALARM → verdict flow vs `voting/ranking.ts` +
`reports/verdict.ts`; reactions' ranking-inertness restated verbatim — with cult display labels
applied throughout (FALSE WITNESS, not HAMBURGER LIAR; The Anointed Wiener, not Top Dog) as
**display copy only**, no code identifier touched. The old page's dangling "← Back to your kennel"
link (to the retired `/snacktum-snacktorum` hub) was **dropped entirely** — the mock ends at the
doxology and the shell provides nav — so **DW-038 is closed by this task**. The shell nav was
updated `help` → `catechism` (href + active-check + link, desktop + mobile), and the retired-hub
comment path `/help` → `/catechism`. The page is now at `/snacktum-snacktorum/catechism`, and with
this rename **every in-app leaf is renamed** (procession / shrine / litter / epistles / summon /
tribunal / catechism) — **no pre-rename leaves remain.** This was a skin-not-skeleton pass at its
lightest: no migration / new dependency / new architecture-decision row / code-identifier rename
(`hamburger_liars` / `summarizeLiarBrand` / `ProfilePoliceBanner` unchanged; decisions stay #1–#29,
L2 preserved). Reviewer APPROVE, **0 fix cycles** — the one finding, a stale
`/snacktum-snacktorum/help` path + pre-rebrand "How Top Dog works" copy in `README.md`, was minor
and folded into bookkeeping. `pnpm check` 0/0, `pnpm lint` clean, `pnpm test` 946/946, `@smoke`
5/5, `@security` 93/93. Did NOT close the milestone (M8 is 15/16 — **only TASK-101 The Lost Pilgrim
remains; completing it closes the milestone**).

**The sixteenth — and FINAL — M8 task landed: TASK-101 — The Lost Pilgrim (16/16; PR #140
`0d6a63d`, merged 2026-06-23), the milestone's one brand-new page, which CLOSES M8.** A root
error / 404 boundary at `src/routes/+error.svelte`, rebuilt from `docs/design/snacktum-snacktorum/pages/The Lost
Pilgrim.dc.html`. Unlike every other M8 page this is not a re-slug or a markup swap over an
existing route — there was no error boundary before, so it is **net-new markup with no
`+page.server.ts`, no migration, no new dependency, and no new architecture-decision row**
(decisions stay #1–#29, L2 preserved). It is driven entirely by `page.status` + `page.error`
from `$app/state`: a **404** renders the designed "Thou Hast Strayed" lost-pilgrim treatment,
**any other status** renders "A Disturbance in the Tube" with the actual status numeral; the
CTA home and the brand mark route via `resolve('/')`. **‼️ The security crux is what is NOT
rendered:** the boundary shows **only friendly cult copy plus the numeric status** —
`page.error.message` is **never** put on screen, and there is no `{@html}`. That makes the L2
no-internal-detail-leak posture **structural** — a server `error(...)` message cannot leak
through a page that has no code path reading it. The director's one fix was a lint fix, not a
logic change (a bare `href="/"` → `resolve('/')` for `svelte/no-navigation-without-resolve`).
...
root boundary covers every route). Reviewer APPROVE, 1 fix cycle. `pnpm check` 0/0, `pnpm lint`
clean, `pnpm test` 946/946, `@smoke` 5/5, `@security` 93/93.

**🏁 Milestone M8 is COMPLETE (16/16) — closed 2026-06-23, tag
`milestone-08-snacktum-snacktorum-rebrand`.** The full rebrand landed as a **skin-not-skeleton**
pass: "Top Dog" became the hot-dog **cult** app "Snacktum Snacktorum", the champion title became
"The Anointed Wiener" (copy only), the in-app route prefix moved `/app` → `/snacktum-snacktorum`
with every leaf re-slugged to a cult name, and every user-facing page was rebuilt from its
delivered design mockup — **without renaming a single DB table, RPC, TS symbol, or piece of
infrastructure, and with decisions #1–#29 + the L2 profile preserved throughout.** The whole
milestone added exactly **one migration** (TASK-094's prune retirement, the source of the one
new decision row #29) and **zero new dependencies**. The **M8-close wiring audit passed** — every
new M8 export/component (`computeBadges`, `loadShrineStats`, `getCurrentChampion`,
`listAnointmentsForProfile`, `createFormValidation`, `errorSlideFade`, `Reliquary`, `Sigil`,
`parseSigilId`; the `+error.svelte` boundary) has a non-test consumer, with no code orphans (the
known orphaned brand/sigil **assets** are tracked as DW-031, non-blocking). The standing hosted
bring-up op (batch the two M7 migrations + the prune-retirement migration + the recovery-template
config in one `supabase db push`) remains the user's async action — no auto-pause risk. **Next:
M9 — Operator / Admin Dashboard, still a stub; activate via the planner when ready.**
...
See [[workflow/handoffs/handoff-008]] for session context.
...
3. **Hosted-push gate (DEFERRED to TASK-054, user-gated ops).** M5 adds **three** unpushed
   ...
   in [[workflow/tasks/deferred]]; see Process notes.
...
2097152` (2 MiB) is the TS-side constant in
   ...
   **preserves and does not touch**) under the L2 defense-at-the-DB posture.
   ...
   remains an accepted v1 residual, kept tracked in [[workflow/tasks/discovered]]). An
...
6. **M8 RE-SCOPE — rebuild-from-design + re-slug (PR #111 `0991846`, planner,
   ...
   **REBUILT FROM their `docs/design/snacktum-snacktorum/pages/*.dc.html` mockups** (a per-page presentational
   ...
   089→094-R Reliquary). See [[workflow/tasks/milestone-08-snacktum-snacktorum-rebrand]] § Slug Map.
...
- **Task queue migrated to the indexed per-milestone layout (2026-06-11).** The
  ...
  its own `workflow/tasks/milestone-NN-slug.md` file (M2 active, M3–M7 pre-exploded),
  cross-milestone logs are `workflow/tasks/discovered.md` + `workflow/tasks/deferred.md`, and the
  completed pre-migration milestones (M0, M1) are grandfathered into
  `workflow/tasks/TASKS-ARCHIVE.md`. New completed milestones freeze in their own file rather than
...
- **Session continuity — M4 built + closed in one session
  ([[workflow/handoffs/handoff-011]], 2026-06-16).** M4 (pre-exploded) was activated,
...
- **M5 hosted-push gate — DONE 2026-06-17 (TASK-054).** The user ran a single
  ...
  TS), so there is no M6 hosted-push gate. See [[workflow/tasks/deferred]] (row marked
  `done`) and [[workflow/handoffs/handoff-014]].
...
| M8 — Snacktum Snacktorum rebrand | rebrand "Top Dog" → the hot-dog CULT app; app shell + nav, auth cluster (sign-in / reset / ritual sign-up), per-page rebuilds from the design mockups + cult slug renames, "Anoint" re-theme, error/404, cult theme | ✅ complete (16/16) — 2026-06-23, tag `milestone-08-snacktum-snacktorum-rebrand` | **User-pivot milestone, SKIN NOT SKELETON** (user-facing copy/markup/CSS/new pages only — NO code-identifier or infra rename; "Top Dog" → "The Anointed Wiener" is **copy only**). Scoped in PR #90; auth-cluster + app-shell designs delivered in PR #91. **‼️ RE-SCOPED 2026-06-19 (PR #111, user-directed) — rebuild-from-design + re-slug.** The remaining pages are now **REBUILT FROM their `docs/design/snacktum-snacktorum/pages/*.dc.html` mockups** (per-page presentational rebuild of `+page.svelte`, preserving each `+page.server.ts` load+actions) **and the in-app routes RE-SLUGGED to cult names** (`app` → `snacktum-snacktorum`; feed→`procession`, dogs→`litter`, profile/[handle]→`shrine/[handle]`, messages→`epistles`, invite→`summon`, court→`tribunal`, help→`catechism`). The **four auth slugs are KEPT descriptive** (`/sign-in` / `/sign-up` / `/forgot-password` / `/reset-password`); the onboarding rite lives at `/sign-up`. Pre-launch (invite-only, not deployed) → **no old→new redirects.** A **scope decision, NOT a numbered decision row** (the slug re-scope adds no row; the one new numbered row of the milestone, TASK-094's **#29** mustard_sprays-retention / prune retirement, has now LANDED → table at **#29**). Task set re-numbered: **15/16 complete** — TASK-087 (base cult theme, PR #99 `dcce8c3`), TASK-080 (app shell + persistent nav, PR #101 `544b7be` — bare `/app` hub retired, **`TopDogPrivilegesNotice` RETIRED**), TASK-083 (forgot/reset password — **6-digit OTP** + code-emitting recovery email template, PR #103 `3e236be`), TASK-082 (real `/sign-in` — `signInWithPassword`, non-enumerating, PR #105 `5445002` — **closes the auth cluster, functional end-to-end**), TASK-092 (the Snacktum Onboarding RITE at `/sign-up`, PR #112 `a5fd084` — the first rebuild-from-design page), **TASK-090 (the foundational slug refactor, PR #115 `38c8844`, 2026-06-20 — checkpoint tag `checkpoint-2026-06-20-pre-slug-refactor`)**: moved the in-app route **prefix** `/app` → `/snacktum-snacktorum` (directory + the `hooks.server.ts` auth-guard prefix so the protected area stays guarded in lockstep), repointed the root redirect to `/snacktum-snacktorum/feed` (the live leaf — TASK-091 renamed it to `procession`, since landed), re-wired the shell links, and swept the `/app/...` references in code + tests + live docs; **leaf names UNCHANGED** (deferred to the per-page rebuilds), the four gate slugs unchanged, the `/sign-in` redirect targets kept; 0 fix cycles, no migration / no dep / no new decision row. **TASK-091 (The Procession, PR #117 `dffaee5`, 2026-06-20 — the first rebuild-from-design IN-APP page):** the feed `+page.svelte` rebuilt from `docs/design/snacktum-snacktorum/pages/The Procession.dc.html`, the **load + all 6 actions preserved** (skin-not-skeleton, ~95% rename), the champion ribbon plumbed in via a widened `listVotableDogs` select (`is_current_top_dog` → derived `championDogId`, RLS-scoped read of the non-client-writable crown column, decision #25), the first leaf renamed `feed` → `procession` (the feed is now at `/snacktum-snacktorum/procession`; **only the feed leaf moved**), and the README/CLAUDE `/feed`→`/procession` current-state docs swept; champion title copy "The Anointed Wiener" (copy only). Reviewer APPROVE, 0 fix cycles (one mid-task escalation resolved by director decision), no migration / no dep / no new decision row. **TASK-093 (The Shrine, PR #122 `851fa0e`, 2026-06-22 — the second rebuild-from-design IN-APP page):** the profile `+page.svelte` rebuilt from `docs/design/snacktum-snacktorum/pages/The Shrine.dc.html`, the **load + all 3 actions (`spray` / `post` / `deleteMessage`) preserved** (skin-not-skeleton), the leaf renamed `profile` → `shrine` (the profile is now at `/snacktum-snacktorum/shrine/[handle]`; **only the `profile` leaf moved**), and a **derived stat ledger** added — new read-only `src/lib/features/profiles/stats.ts` (`loadShrineStats(...)` → `ShrineStats`, `EMPTY_SHRINE_STATS`) computing aggregates over existing tables with no schema/write path; seven counts stay RLS-scoped, the eighth ("Disciples Summoned" / redeemed invites) runs on the **service client** as a `head:true` count after `safeGetSession()` because `invites` is owner-scoped-RLS (`invites_select_own`) so an RLS count returns 0 cross-member — **generalizing the decision #27 service-client-after-gate pattern to a cross-member aggregate** (a head count ships no rows → no exposure widening). **2 fix cycles:** a tester-caught P0 (wall composer `<textarea name>` mismatched the `post` action's `formData.get('body')` → silent empty posts; fixed to `name="body"`) and reviewer two majors ("Disciples Summoned" RLS undercount → service-client head count; wall textarea themed validation never fired because the `<label>` was a sibling not a wrapper → nested it so `fieldLabel()`'s `closest('label')` resolves). The validation module was widened to validate `<textarea>` and `validationMessage.ts` gained the "Word upon the Shrine" themed label. Reviewer APPROVE (after 1 REQUEST_CHANGES round); `pnpm check` 0/0, `pnpm lint` clean, `pnpm test` 861/861, `@smoke` 5/5, `@security` 94/94; no migration / no dep / no new decision row. Discovered DW-035 (no jsdom vitest project → DOM-touching `.svelte.ts` logic unit-untested). TASK-092 details: a multi-step rite (Summoned → Inscribe → Choose Thy Sigil → Renounce → Received) that **absorbs and deletes** `(protected)/app/onboarding/`; profile forged at the Sigil step, Renounce a pure-UI oath, `createProfile` **returns `{ created, handle }` instead of redirecting** so the client advances without re-running `load` (which would redirect on the new profile); sigil stored as `sigil:<id>` in `avatar_path` (new `Sigil.svelte` + `sigils.ts`, no migration); Ordo Sancti Tubi **seal + wordmark header unified across all four auth/gate pages** via shared `.gate-mark`/`.gate-header`. Reviewer APPROVE; `pnpm check` 0, `pnpm lint` clean, `pnpm test` **830**, `@smoke` 5/5, `@security` 94/94. **All design questions RESOLVED** (2026-06-18 + 2026-06-19 OQ-5 dog-detail = **"The Relic"** + OQ-2 the five Anoint sub-decisions). **Ad-hoc M8 follow-up (not a queued task): themed inline form-validation landed and is now an app-wide CANON** (PR #109 `6c00c1c` — `createFormValidation()` rune + `validationMessage.ts` + `errorSlideFade`, replacing the native HTML5 bubble; rollout to remaining in-app forms tracked DW-032). **TASK-094 (Anoint mustard re-theme, PR #124 `645373a`, 2026-06-22 — the ONLY M8 migration):** the Top-Dog mustard mechanic re-skinned as "Anoint" (splat visual + decay shortened **24h → 6h** in `MUSTARD_LIFESPAN_MS`, overlay-only) + a **persisting wall-notice render-derived from the FULL spray history** (new `listAnointmentsForProfile`, cap 200) that outlives the 6h fade; the daily prune was **RETIRED** (`20260622120000_retire_mustard_prune.sql` function-only DROP of `prune_mustard_sprays()` + the keep-alive prune step removed in lockstep), so `mustard_sprays` is now **append-only** — **the milestone's one new numbered architecture-decision row, #29**. 2 reviewer fix cycles (notice persistence derived from the 6h overlay → full history; stale comment + copy). **TASK-094-R (The Reliquary, PR #126 `870e401`, 2026-06-22 — closes the Shrine cluster):** a purely DERIVED badge/honors module — new pure `src/lib/features/badges/badges.ts` (`computeBadges`) + `Reliquary.svelte` shelf, every badge computed at render time from already-loaded facts (no migration / schema / RPC / write path / second service-client read), composing decisions #12/#13/#15/#27, **no new decision row**; a derived sub-module, **not counted in the `/16`**. **TASK-095 (Your Litter, PR #128 `4cab7df`, 2026-06-22):** the own-dogs gallery + upload rebuilt from `Your Litter.dc.html`, `+page.server.ts` preserved (own-gallery stays fully RLS-scoped — no service client), the whole `dogs` folder `git mv`'d → `litter` (the `[id]` subfolder rode along rename-only), themed-validation CANON on the `photo` field. **TASK-096 (The Relic, PR #130 `d07315f`, 2026-06-22 — the cleanest of the milestone, a markup-only rebuild):** the dog-detail rebuilt in place at `litter/[id]` from `The Relic.dc.html`, `+page.server.ts` **byte-identical** (the load-bearing decision-#27 service-client signed-URL load left wholly untouched). **TASK-097 (Epistles + Whispers, PR #132 `8764287`, 2026-06-22 — the DM cluster):** both DM pages rebuilt from `Epistles.dc.html` + `Whispers.dc.html`, both `+page.server.ts` preserved (inbox a pure rename; thread only the self-thread redirect literal `/messages`→`/epistles`), the whole `messages` folder `git mv`'d → `epistles` (+`epistles/[handle]`), themed-validation CANON on the compose box, surfaced pre-existing DW-038 (The Catechism's "← Back to your kennel" links the retired hub). **TASK-098 (Summon a Frank, PR #134 `dc7a229`, 2026-06-23 — the sixth rebuild-from-design in-app page):** the invite `+page.svelte` rebuilt from `Summon a Frank.dc.html` (main content only — header/nav chrome stays in the app shell), the DSL `<sc-if>` states ported to Svelte 5 runes bound to the REAL `?/create` form contract (idle → minting → minted → error), a copy-to-clipboard affordance added (net-new over the original page); the whole `invite` folder `git mv`'d → `summon` so `+page.server.ts` is **byte-identical / R100 rename** (the invite-mint action, decisions #17/#22/#23, untouched), the test renamed `invite-action.test.ts` → `summon-action.test.ts`; no migration / dep / decision row / code-identifier rename. Reviewer APPROVE, 0 fix cycles (two minor optional a11y findings deferred to the tweak session); surfaced DW-039 (the Summon page has no inbound nav link, and the shell's "＋ Summon a Frank" button targets upload, not this page — a label/reachability clash). **TASK-099 (The Tribunal, PR #136 `94de595`, 2026-06-23 — the seventh rebuild-from-design in-app page):** the court `+page.svelte` rebuilt from `The Tribunal.dc.html` as The Tribunal of the Holy Tube (main content only), the mock's per-case states mapped onto the REAL `dog.verdict` (the reused `ConfirmedHamburgerStamp` re-placed per the mock); the whole `court` folder `git mv`'d → `tribunal` so `+page.server.ts` is **byte-identical / R100 rename** — the **double gate** (UI crown redirect + the `render_burger_verdict` RPC's authoritative `is_current_top_dog` re-check, decision #25), the anonymous flagged-dog aggregate (service-client-after-gate, decision #27/#33 signed URLs), and the `rule` action all untouched, reporter anonymity intact. Reviewer APPROVE, 0 fix cycles; surfaced DW-040 (the `shell-nav-court` CSS class left as-is — a cosmetic nit). **TASK-100 (The Catechism, PR #138 `e42087a`, 2026-06-23 — the eighth and FINAL rebuild-from-design in-app page):** the **static** how-it-works `+page.svelte` rebuilt from `The Catechism.dc.html` as The Catechism (six illuminated Article sections + a doxology, main content only) + the final leaf `help` → `catechism` (the whole folder `git mv`'d; a static page, so **no `+page.server.ts`** exists). **Accuracy-checked against source, not the mock:** the mockup's stale "fades over about a day" Anoint copy was **corrected to ~6h** (`MUSTARD_LIFESPAN_MS`, decision #29), and FALSE WITNESS (7d) / HERETIC (permanent) / the vote+crown rules / reactions' ranking-inertness / the report → ALARM → verdict flow were all cross-checked against `voting/ranking.ts` + `reports/verdict.ts`; cult display labels are copy only (no code identifier touched). The dangling "← Back to your kennel" link to the retired hub was dropped (**DW-038 closed**). With this rename **every in-app leaf is renamed** — no pre-rename leaves remain. No migration / dep / decision row / code-identifier rename. Reviewer APPROVE, 0 fix cycles (one minor README path-drift finding folded into bookkeeping). **No new dependency this session; the one new decision row is #29.** **TASK-101 (the Lost Pilgrim, PR #140 `0d6a63d`, 2026-06-23 — the FINAL task, CLOSING M8):** the brand-new root `src/routes/+error.svelte` boundary (no `+page.server.ts`), driven by `page.status` + `page.error` — 404 → "Thou Hast Strayed", any other status → "A Disturbance in the Tube" + the actual status numeral; renders ONLY friendly copy + the numeric status, **never `page.error.message`** (L2 no-leak, structural; no `{@html}`). 1 fix cycle — a `@smoke`-caught interaction (the `litter/[id]` 404 now renders this page, suppressing its message, so `feed-detail.e2e.ts` was strengthened to assert the designed page + a positive no-leak guard), resolved by hardening the test, not the page; director lint fix `href="/"` → `resolve('/')`. No migration / dep / decision row. **🏁 M8 CLOSED 2026-06-23 (16/16), tag `milestone-08-snacktum-snacktorum-rebrand`** — wiring audit passed (no code orphans; orphaned brand/sigil assets tracked DW-031); the whole rebrand added exactly one migration (TASK-094's) + zero new deps + one new decision row (#29). See [[workflow/tasks/milestone-08-snacktum-snacktorum-rebrand]] |
