...
Continued **Milestone M5 — Walls & DMs** ([[workflow/tasks/milestone-05-walls-dms]]),
...
- `CLAUDE.md` — MODIFIED (this handoff): Project Map latest-handoff pointer →
  `[[workflow/handoffs/handoff-012]]`.
- `workflow/tasks/discovered.md` — MODIFIED (this handoff): DW-018 logged.
- `workflow/handoffs/handoff-012.md` — NEW (this file).
...
- [ ] **DW-018** — `listConversations` / `listThread` in
      ...
      TASK-051 review (PR #62). Disposition `open` in [[workflow/tasks/discovered]].
...
1. **[user-gated — do first] Merge PR #62** (TASK-051, APPROVED). Then the
   ...
   [[workflow/tasks/milestone-05-walls-dms]], tick both ACs), then **CLOSES M5**: run the
...
- [[workflow/tasks/milestone-05-walls-dms]] — the active milestone; TASK-051 status stays
...
See [[workflow/handoffs/handoff-011]] for prior session context.
