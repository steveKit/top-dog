...
| M8 — Snacktum Snacktorum rebrand | rebrand "Top Dog" → the hot-dog CULT app; app shell + nav, auth cluster (sign-in / reset / ritual sign-up), profile redesign, "Anoint" re-theme, error/404, cult theme | active — BUILDING (4/10) | **User-pivot milestone, SKIN NOT SKELETON** (user-facing copy/markup/CSS/new pages only — NO code-identifier or infra rename; "Top Dog" → "The Anointed Wiener" is **copy only**). Scoped in PR #90; auth-cluster + app-shell designs delivered in PR #91 (`design/pages/` Log In / Reset Password / Snacktum Onboarding / App Chrome; `design/avatars/` 5 SVG sigils; `design/assets/` brand marks + favicons; `design/page-design-prompts.md` 11 prompts). **BUILD activated 2026-06-19. 2/10 complete:** TASK-087 (base cult theme, PR #99 `dcce8c3` — tokenized dark-temple CSS + self-hosted Cinzel/Cormorant fonts; 1 fix cycle) and TASK-080 (global app shell + persistent nav, PR #101 `544b7be` — persistent `(protected)/app/+layout.svelte`, `/` → `/app/feed` repoint, bare `/app` hub retired; **`TopDogPrivilegesNotice` intentionally RETIRED**, `pnpm test` 783 → 775; 1 fix cycle). **All design questions RESOLVED** — 2026-06-18: the ritual sign-up rite (absorbing onboarding), the 5-sigil avatar (sigil id in the existing `avatar_path` — no migration / no storage upload; uploaded avatars deferred), the dark-temple theme (`#17120e` + Mustard Gold `#E0A82E`), self-hosted Cinzel/Cormorant woff2 (no dependency), the 6-digit-OTP reset flow; **2026-06-19: OQ-5** (dog-detail page = **"The Relic"**) **+ OQ-2** (all five Anoint sub-decisions — keep gated, re-theme the spray / no merge, **splat** visual, decay shortened to **~6h**, a 24h-rolling-stack **persisting** wall notice). **‼️ Posture change (OQ-2 Option A, user-approved): TASK-086 will RETIRE `prune_mustard_sprays()` so the persisting wall-notice rows survive — so M8 now carries ONE migration (TASK-086 prune retirement) + a keep-alive workflow edit + a LIKELY new architecture-decision row #29 (mustard_sprays retention).** Recorded as a **plan** — the decision table stays at **#28** until TASK-086 lands. **3/10 (2026-06-19): TASK-083 (forgot/reset password, PR #103 `3e236be`)** added the password-recovery cluster — `/forgot-password` (`resetPasswordForEmail`, neutral non-enumerating) + `/reset-password` (**6-digit OTP** → `verifyOtp(type:'recovery')` → `updateUser`; `MIN_PASSWORD_LENGTH` 8 + confirm); 1 fix cycle added a code-emitting recovery email template (`supabase/templates/recovery.html` `{{ .Token }}` + `[auth.email.template.recovery]` / `otp_length = 6`), director-verified by a live Mailpit round-trip; `pnpm test` 793; **no migration / no new dependency / no new decision row**; adds a hosted CONFIG item to the standing gate (set hosted recovery template or production sends a link not a code); DW-029 logged (`MIN_PASSWORD_LENGTH` duplication). TASK-087/TASK-080/TASK-083 each added no migration / no new dependency / no new decision row. **4/10 (2026-06-19): TASK-082 (sign-in, PR #105)** closed the auth cluster; **interim gate-page visual polish + brand assets landed ad-hoc (PR #107, not a queued task).** Next: TASK-081 (copy) / TASK-085 (profile) / TASK-084 (ritual) / TASK-086 (Anoint) / TASK-088 (error) / TASK-089 (Reliquary). See [[tasks/milestone-08-snacktum-snacktorum-rebrand]] |
